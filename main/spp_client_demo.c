 ...
 *  @brief      Register callback function to the gap module
 *  @details    For BLE connection and advertising events
 * 
    ...
    switch(event){
	    ...
	    case ESP_GAP_BLE_SCAN_STOP_COMPLETE_EVT:
	        ...
	        if (is_connect == false) {
	            ...
	            esp_ble_gattc_enh_open(gl_profile_tab[PROFILE_APP_ID].gattc_if, &creat_conn_params);
	        }
...
static void esp_gattc_cb(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param) {
    ...
    do {											/* @open  is this lazy do {} while(0); wrapper useful? */
		
        int idx;
        
        for(idx = 0; idx < PROFILE_NUM; idx++) {
			
            if((gattc_if == ESP_GATT_IF_NONE) || (gattc_if == gl_profile_tab[idx].gattc_if)) { 		/* ESP_GATT_IF_NONE, not specify a certain gatt_if, need to call every profile cb function */
